...
public sealed class CodexAnswerProvider : IAnswerProvider
{
    ...
    public event Action<AccountRateLimits?>? RateLimitsChanged;

    public static CodexAnswerProvider CreateDefault()
    {
        var executable = Environment.GetEnvironmentVariable("CLIPASK_CODEX_PATH");
        if (string.IsNullOrWhiteSpace(executable))
            executable = Path.Combine(AppContext.BaseDirectory, "codex.exe");
        var state = Environment.GetEnvironmentVariable("CLIPASK_STATE_DIR");
        if (string.IsNullOrWhiteSpace(state))
            state = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ClipAsk");
    ...
    public async Task<AccountRateLimits?> GetRateLimitsAsync(CancellationToken cancellationToken = default)
    {
        await EnsureStartedAsync(cancellationToken).ConfigureAwait(false);
        var rpc = connection ?? throw new InvalidOperationException("Codex transport is unavailable.");
        ...
        if (!connected.IsConnected)
            return null;
        var response = await rpc.RequestAsync("account/rateLimits/read", null, cancellationToken).ConfigureAwait(false);
        return CodexPolicy.ReadRateLimits(response);
    }
    ...
    public async Task<CodexModelSelection?> GetAutomaticModelAsync(CancellationToken cancellationToken = default)
    {
        await EnsureStartedAsync(cancellationToken).ConfigureAwait(false);
        var rpc = connection ?? throw new InvalidOperationException("Codex transport is unavailable.");
        var connected = account?.IsConnected == true ? account : await GetAccountAsync(cancellationToken).ConfigureAwait(false);
        if (!connected.IsConnected)
            return null;
        return CodexPolicy.SelectModel(await GetModelCatalogAsync(rpc, cancellationToken).ConfigureAwait(false));
    }
    ...
    private async Task ProduceAnswerAsync(ReadOnlyMemory<byte> png, AnswerRequestOptions options, ChannelWriter<AnswerUpdate> output, CancellationTokenSource linked, CaptureTiming? timing)
    {
        ...
        try
        {
            Publish(new(AnswerUpdateKind.Status, "Asking ChatGPT…"));
    ...
    private static async Task InitializeAsync(JsonRpcConnection rpc, CancellationToken cancellationToken)
    {
        ...
        {
            clientInfo = new { name = "clipask_desktop", title = "ClipAsk", version = "0.1.0" }
        });
    ...
    private void OnConnectionNotification(string method, JsonElement parameters)
    {
        ...
        {
            RateLimitsChanged?.Invoke(CodexPolicy.ReadRateLimits(parameters));
        }
    }
