...
public static class ResultLayoutCalculator
{
    ...
    private static ResultLayout CalculateStacked(ResultLayoutInput input)
    {
        ...
        var answerWidth = Math.Max(1, windowWidth - Insets);
        ...
        var maximumAnswerHeight = Math.Min(maxWindowHeight * 0.62, availableAnswerHeight);
        var maximumWindowHeight = Math.Min(maxWindowHeight, HeaderHeight + Insets + previewHeight + Gap + maximumAnswerHeight);

        return new(ResultLayoutMode.Stacked, windowWidth, windowHeight, previewWidth, previewHeight, answerWidth, answerHeight, maximumWindowHeight);
    }
    ...
    private static ResultLayout? CalculateSplit(ResultLayoutInput input)
    {
        ...
        return new(ResultLayoutMode.Split, windowWidth, windowHeight, previewWidth, previewHeight, answerWidth, answerHeight, maxWindowHeight);
    }
}
