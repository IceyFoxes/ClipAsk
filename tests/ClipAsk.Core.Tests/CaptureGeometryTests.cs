...
public sealed class CaptureGeometryTests
{
    ...
    [Fact]
    public void SelectionTopLeftPlacementUsesTheNormalizedCropOriginAndClamps()
    {
        var work = new PixelRect(-1920, 0, 1920, 1080);

        Assert.Equal(new PixelRect(-1500, 120, 600, 400), PanelPlacement.PlaceAtSelectionTopLeft(new(-1500, 120, 300, 200), work, 600, 400));
        Assert.Equal(new PixelRect(-600, 680, 600, 400), PanelPlacement.PlaceAtSelectionTopLeft(new(-100, 900, 100, 100), work, 600, 400));
        Assert.Equal(new PixelRect(-1920, 0, 1920, 1080), PanelPlacement.PlaceAtSelectionTopLeft(new(-1500, 120, 300, 200), work, 2400, 1200));
    }

    [Fact]
    public void CenteredPlacementUsesTheCaptureMonitorWorkArea()
    {
        var work = new PixelRect(-1920, 40, 1920, 1040);

        Assert.Equal(new PixelRect(-1320, 360, 720, 400), PanelPlacement.PlaceCentered(work, 720, 400));
        Assert.Equal(work, PanelPlacement.PlaceCentered(work, 2400, 1200));
    }

    [Fact]
    public void GrowingAManuallyMovedWindowKeepsItVisible()
    {
        var work = new PixelRect(-1920, 40, 1920, 1040);

        Assert.Equal(new PixelRect(-720, 280, 720, 800), PanelPlacement.KeepVisible(new(-720, 700, 720, 200), work, 720, 800));
        Assert.Equal(new PixelRect(-1920, 40, 1920, 1040), PanelPlacement.KeepVisible(new(-2500, -200, 500, 300), work, 2400, 1200));
    }
    ...
    public void ResultLayoutChoosesExpectedModeAndPreservesAspectRatio(double width, double height, ResultLayoutMode expected)
    {
        ...
        Assert.True(layout.MaximumWindowHeight >= layout.WindowHeight);
        Assert.True(layout.MaximumWindowHeight <= 1048);
    }
    ...
    [Fact]
    public void StackedResponsePaneUsesTheNaturalContentWidth()
    {
        var layout = ResultLayoutCalculator.Calculate(new(1600, 120, 1920, 1080));

        Assert.Equal(ResultLayoutMode.Stacked, layout.Mode);
        Assert.Equal(layout.WindowWidth - 32, layout.AnswerWidth);
    }
